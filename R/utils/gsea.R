...
prepare_deseq2_dfs_for_gsea <- function(data_list,
  ...
  if (save_missing_genes) {
    write.table(missing_genes[[1]],
      file = missing_gene_destination,
...
make_gsea_bubble_plot_RT <- function(fgsea_output_list,
  ...
  combined_df$perturbations <- gsub(
    "_non-targeting_noRT", "",
  ...
  df_ordered <- combined_df %>%
    arrange(
      match(pathway, rownames(nes_df)[row_order]),
  ...
  if (log_fc_overlay) {
    bubble_plot <- bubble_plot +
      geom_point(aes(
        y = pathway,
        size = log10padj,
        color = perturb_cond_pathway_means
      ), alpha = 1.0) +
    ...
    bubble_plot <- bubble_plot + geom_point(aes(
      y = pathway,
      size = log10padj,
      color = NES
    ), alpha = 1.0) +
  ...
  bubble_plot <- bubble_plot +
    geom_point(
      data = subset(df_ordered, padj < 0.05),
      aes(y = pathway, size = log10padj, shape = "adjp < 0.05"),
      color = "black", fill = NA, alpha = 0.8
    ) +
    geom_point(
      data = subset(df_ordered, is.na(pval) | is.na(padj) | is.na(log10padj)),
      aes(y = pathway),
    ...
    # Add the RT layer
    geom_rect(data = rt_df, aes(
      xmin = numeric_xmin, xmax = numeric_xmax,
      ymin = ymin, ymax = ymax, fill = RT
    ), inherit.aes = FALSE) +
...
make_bubble_plot <- function(fgsea_output_list,
                             color_overlay = FALSE,
                             color_lookup_matrix = NULL,
                             normalization_scheme = "condNormalized",
                             color_label = "Effect Size",
                             color_limits = NULL,
                             plot_title = "Enrichment of Hallmark pathways across perturbations",
                             plot_subtitle = paste(normalization_scheme, "clustered within RT", sep = ", ")) {
  # Given a list of GSEA results, generate a bubble plot for it. This function
  # currently supports plotting clusters within RT conditions for
  # condNormalized and noRTNormalized outputs.
  # - @param: color_lookup_matrix: By default, the bubble plot will be colored by the "NES" column of
  #   the fgsea_output_list dataframes. To overlay a different coloring,
  #   pass in a perturb x pathway matrix whose entries contain values for
  #   coloring.

  if (!normalization_scheme %in% c("condNormalized", "noRTNormalized")) {
    stop("Normalization scheme not permitted!")
  }
  if (color_overlay && is.null(color_lookup_matrix)) {
    stop("Must pass in a color lookup matrix when trying to overlay colors.")
  }
  ...
  combined_df <- bind_rows(fgsea_output_list, .id = "perturbations") %>%
    mutate(
      ...
      perturbations = gsub("_non-targeting_(no)?RT$", "", perturbations),
      RT = ifelse(grepl("_RT", perturbations), "RT", "noRT")
    ) %>%
    group_by(pathway) %>%
  ...
  nes_df <- combined_df %>%
    select(pathway, perturbations, NES) %>%
    pivot_wider(names_from = perturbations, values_from = NES) %>%
    column_to_rownames("pathway")

  row_order <- hclust(dist(nes_df))$order
  row_order <- rownames(nes_df)[row_order]

  col_orders <- lapply(c("RT", "noRT"), function(rt_condition) {
    cols <- grep(sprintf("_%s$", rt_condition), colnames(nes_df), value = TRUE)
    col_df <- nes_df[cols]
    if (ncol(col_df) > 1) {
      ordering <- hclust(dist(t(col_df)))$order
      col_order <- colnames(col_df)[ordering]
    } else {
      print("Only one column detected, skipping column-wise clustering...")
      col_order <- colnames(col_df)
    }
    return(col_order)
  })

  col_order <- c(col_orders[[1]], col_orders[[2]])
  nes_df <- nes_df[, col_order]

  df_ordered <- combined_df %>%
    mutate(
      pathway = factor(pathway, levels = row_order),
      perturbations = factor(perturbations, levels = colnames(nes_df)),
      significance = ifelse(padj < 0.05, "padj < 0.05", "NA")
    )

  rt_df <- data.frame(
    xmin = unique(df_ordered$perturbations),
    xmax = unique(df_ordered$perturbations),
    ymin = max(as.numeric(df_ordered$pathway)) + 1,
    ymax = max(as.numeric(df_ordered$pathway)) + 2,
    RT = ifelse(grepl("_RT", unique(df_ordered$perturbations)), "RT", "noRT"),
    numeric_xmin = as.numeric(factor(unique(df_ordered$perturbations))) - 0.5,
    numeric_xmax = as.numeric(factor(unique(df_ordered$perturbations))) + 0.5
  )

  term_color_map <- c(RT = "darkred", noRT = "darkblue")
  if (color_overlay) {
    df_ordered <- df_ordered %>%
      mutate(color_value = map2_dbl(
        as.character(pathway), as.character(perturbations),
        ~ color_lookup_matrix[.x, .y]
      ))
  } else {
    df_ordered <- df_ordered %>%
      mutate(color_value = NES)
  }

  bubble_plot <- ggplot(df_ordered, aes(x = perturbations, y = pathway))
  bubble_plot <- bubble_plot +
    geom_point(aes(
      y = pathway, size = log10padj,
      color = color_value
    ), alpha = 1.0) +
    scale_color_gradient2(
      low = "blue",
  ...
  bubble_plot <- bubble_plot +
    ...
    labs(color = color_label, size = "-Log10 padj") +
    theme(
      axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
      panel.background = element_blank(),
      panel.grid.major = element_blank(),
  ...
  perturb_dendrograms <- lapply(c("RT", "noRT"), function(rt_condition) {
    cols <- grep(sprintf("_%s$", rt_condition), colnames(nes_df), value = TRUE)
    sub_df <- t(nes_df[, cols, drop = FALSE])
    if (nrow(sub_df) > 1) {
      tree <- as.phylo(hclust(dist(sub_df)))
      p <- ggtree(tree) +
        theme_tree() +
        coord_flip() +
        scale_x_reverse()
    } else {
      print("Only one column detected, skipping dendrogram generation...")
      p <- ggplot()
    }
    return(p)
  })

  module_dendrogram <- ggtree(as.phylo(hclust(dist(nes_df)))) + theme_tree()

  vertical_plot <- (perturb_dendrograms[[1]] | perturb_dendrograms[[2]]) / bubble_plot + plot_layout(heights = c(0.5, 3))
  dendrogram_vertical_plot <- plot_spacer() / module_dendrogram + plot_layout(heights = c(0.85, 1))
