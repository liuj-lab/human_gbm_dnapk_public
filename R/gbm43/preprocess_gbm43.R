# Preprocessing of GBM43 CED in vivo perturb-seq data.
# Updated: 6/13/25.
...
library(reshape2)
library(patchwork)
setwd("/raleighlab/data1/liuj/gbm_perturb/analysis/gbm_pdx_perturb_gbm43_1/")
...
# load all GBM43  data
smpid <- dir(file.path("/raleighlab/data1/liuj/gbm_perturb/cellranger_out_combined/gbm_pdx_perturb_gbm43_1"))

dataGene.list <- list()
sgR.list <- list()

for (i in 1:length(smpid)) {
  df <- Read10X(data.dir = paste("/raleighlab/data1/liuj/gbm_perturb/cellranger_out_combined/gbm_pdx_perturb_gbm43_1/", smpid[i], "/outs/filtered_feature_bc_matrix/", sep = ""))
...
plot <- SCpubr::do_DimPlot(dat.sub,
  reduction = "umap", group.by = paste("SCT_snn_res.", res, collapse = "", sep = ""), label = TRUE, repel = TRUE,
...
# marker heatmap
top10.sub <- markers.sub %>%
  group_by(cluster) %>%
  top_n(10, avg_log2FC)
top25.sub <- markers.sub %>%
  group_by(cluster) %>%
  top_n(25, avg_log2FC)
...
plot <- ggplot(sgRNATbl) +
  ...
  theme_base() +
  theme(
    axis.text = element_text(size = 6), text = element_text(size = 6), axis.text.x = element_text(angle = 0, hjust = 0.5, vjust = 1),
...
plot <- ggplot(df) +
  ...
  coord_flip() +
  theme(
    axis.text = element_text(size = 6), text = element_text(size = 6), axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
...
Heatmap(log(df + 1, 10),
  ...
  top_annotation = HeatmapAnnotation(
    gamma = anno_barplot(sgAnnot$gamma, bar_width = 0.9),
...
plot <- ggplot(df) +
  ...
  theme_base() +
  theme(
    axis.text = element_text(size = 6), text = element_text(size = 6),
    axis.ticks = element_line(colour = "black", size = 0.25), panel.border = element_blank(), panel.background = element_blank(), axis.line = element_line(size = 0.25)
  )
...
write.table(remainingRNATbl, "gbm43_remainingRNATbl.txt", col.names = NA, sep = "\t", quote = FALSE)